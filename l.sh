...
Cashier=$(everdev c d ./artifacts/Cashier.abi.json -v 100e9 | grep address | cut -d':' -f3 | xargs)
...
# set "static" vars
 ...
 SlotDebot=$(everdev contract deploy ./artifacts/SlotDebot.abi.json --value 3e8 -d gameCode:$gameCode,gameDeployer:$gameDeployer,cashier:$Cashier | grep address | cut -d':' -f3 | xargs)
